...
import com.example.android_advance.ui.forgotPassword.forgetPasswordViewModel

@Composable
fun VerifyOTPScreenRegister(navController: NavController, sendOtp: String) {
    ...
    Surface(
        ...
        Column(
            ...
            VerifyOTPButton(
                ...
                otpCode = otpCode, serverOtp = sendOtp
            )
...
fun VerifyOTPButton(
    ...
    val viewModel = hiltViewModel<SignUpViewModel>()
    var otpError by remember { mutableStateOf(true) }
    if (otpError) {
        Text(
    ...
    Button(
        onClick = { /* Handle button click */
            ...
            if (serverOtp == otpCode) {
                val userRegister = viewModel.store!!.state.userRegister
                viewModel.signUp(
                    userRegister.name!!,
...
fun ResetPasswordDialog(navController: NavController) {
    ...
    if (showDialog.value) {
        AlertDialog(
            ...
            text = { Text(text = "Signup Account Successful") },
            confirmButton = {
                Button(onClick = {
